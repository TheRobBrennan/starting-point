...
- Always prefix new branch names with today's date in the format YYYY.MM.DD/
- CRITICAL: Use the dynamic date command `date +%Y.%m.%d` to get the current date - NEVER hardcode the date
- Example: `git checkout -b $(date +%Y.%m.%d)/feature-name && git push --set-upstream origin $(date +%Y.%m.%d)/feature-name`
